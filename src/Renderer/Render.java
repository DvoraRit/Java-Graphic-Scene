...
public class Render {
	...
	public void renderImage() throws Exception//this function creates a image of the geometries (scene), from the camera view. 
	{
		...
		for(int i=0;i<height;i++) //For each point (i,j) in the view plane
		{
			for(int j=0;j<width;j++)
			{
				...
				{ 
					...
					//intersectionPoints gets all the intersection points with all of the geometries
					Map<Geometry,List<Point3D>> intersectionPoints =getSceneRayIntersections(ray);
					if (intersectionPoints.isEmpty())//if there are no intersection points, writes the background color
						//in this pixel
					...
					else//writes the color in the closest point in the intersection points list.
					{
	...
	{//this function prints grid to the image (not to the path), divides the screen to grid by interval
			for(int j=0;j<imageWriter.getHeight();j++)
				{for(int i=0;i<imageWriter.getWidth();i++)
					{if(i%interval==0||j%interval==0||i==499||j==499)
						imageWriter.writePixel(j, i, Color.white);}}
	}

	private Color calcColor(Geometry geometry,Point3D p ,Ray inRay) throws Exception
	//this function calculates the color in the point p, it sends 0 in order to begin the counting of the recursion 
	//(till the recursion level)
	{
	...
	private Color calcColor(Geometry geometry,Point3D p ,Ray inRay,int level) throws Exception//this function calculates the color in the point p
	{
		try{
			...
			while(lightsIt.hasNext())
			{
				...
				if (!occluded(light, p, geometry)) // if the light touches the point else black
				{ 
					//temp is the sum of all the diffusive light on the geometry
					...
					//temp1 is the sum of all the specular light on the geometry
					Color temp1=calcSpecularComp(geometry.getMaterial().getKs(),
			...
			double Kr = geometry.getMaterial().getKr();//Kt
			...
			if(Kr!=0)//if the geometry is reflected
			{
				...
				Map<Geometry,List<Point3D>> intersectionPointsReflect= getSceneRayIntersections(reflectedRay);//get all the intersection points
				//of this ray
	...
	private Ray constructReflectedRay(Vector normal, Point3D p, Ray inRay) throws Exception{//return a reflected ray from income ray,intersection
	...
	private Ray constructRefractedRay(Vector normal, Point3D p, Ray inRay) throws Exception{//return a refracted ray from income ray,intersection
		//point and the normal of the geometry in this point
		Vector D=new Vector(inRay.getDirection());
	...
	private boolean occluded(Light light, Point3D point, Geometry geometry) throws Exception //return true if the geometry
	//occluded from the light in p-a specific point
	{
	...
	//this function return the closest point to the camera (P0) in the intersectionPoints list
	{
	...
	//the ray hits in them (it checks all the geometries in the scene)
	{
		...
		while (geometries.hasNext())//scans all the geometries in the scene 
		{
	...
	private Color calcDiffusiveComp(Double Kd,Vector geoNormal,Vector L,Color Intensity) throws Exception
	{ // calculates the diffuse light at the geometry's point 
	...
	private Color calcSpecularComp(Double Ks,Vector V,Vector geoNormal,Vector L,double n,Color Intensity) 
	throws Exception {// calculates the specular light at the geometry's point 
